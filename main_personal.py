...
# Removida: from itsdangerous import TimestampSigner, BadTimeSignature, SignatureExpired
...
from fastapi import FastAPI, HTTPException, Query, Body, Path as FastApiPath, APIRouter
from fastapi.responses import FileResponse, HTMLResponse, JSONResponse, Response
from fastapi.staticfiles import StaticFiles
from fastapi.middleware.cors import CORSMiddleware

# Certifica que o código abaixo está no seu main_personal.py
# O código de verificação de licença foi removido ou alterado.
...
# ============================================================
# Licenciamento (REMOVIDO PARA USO PESSOAL)
# ============================================================

def verificar_licenca_local():
    """
    MODIFICADO PARA USO PESSOAL ILIMITADO. 
    A função agora sempre retorna True, ignorando o license.key.
    """
    logging.info("Verificando licença (MODO PESSOAL ILIMITADO)...")
    return {"valido": True, "mensagem": "Licença Ilimitada (Uso Pessoal)"}
...
try:
    ...
    # Define app como None para que o PyQt6 possa exibir a mensagem de erro
    app = None 


def start_server_process(log_path):
    ...
    logging.info(f"PROCESSO SERVIDOR: Tentando iniciar Uvicorn em 127.0.0.1:8000...")
    try:
        # Passa o objeto app, não a string
        uvicorn.run(app, host="127.0.0.1", port=8000, log_level="info", reload=False, log_config=None)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 3. Inicia servidor 
        logging.info("Iniciando processo servidor...")
        # O Uvicorn roda em outro processo para não bloquear a UI (PyQt)
        server_p = multiprocessing.Process(target=start_server_process, args=(LOG_FILE_PATH,), name="ServidorUvicorn")
